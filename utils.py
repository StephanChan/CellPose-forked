...
def fill_holes_and_remove_small_masks(masks, min_size=150):
...
def plot_results_with_input(live, dead, image, luna_path):
    image[:,:,0][dead>0]=255
    image[:,:,1][live>0]=255
    im = Image.fromarray(image.astype(np.uint8))
    im.save(str(Path(luna_path[0]).parent.absolute())+"\\AI.tif")
    fig = plt.figure(figsize=(16, 6))
    ax0 = fig.add_subplot(121)
    ax0.set_title("AI cell counting")
    plt.imshow(image.astype(np.uint8))
    ax1=fig.add_subplot(122)
    ax1.set_title("commercial product cell counting")
    luna = my_io.imread(luna_path[0])
    plt.imshow(luna)
   

def split_live_dead_cells(masks, RFP_path, GFP_path, RThreshold=40, GThreshold=12):
    RFP = my_io.imread(RFP_path[0])[:,:,0]
    GFP = my_io.imread(GFP_path[0])[:,:,1]
    live_mask=np.zeros(masks.shape)
    dead_mask=np.zeros(masks.shape)
    
    ncells=np.max(masks)
    RFP_cell=np.zeros(ncells)
    GFP_cell=np.zeros(ncells)
    for cell in range(ncells):
        RFP_cell[cell]=np.mean(RFP[masks==cell])
        GFP_cell[cell]=np.mean(GFP[masks==cell])
    # plt.hist(RFP_cell, bins=100)
    # plt.hist(GFP_cell, bins=100)
    live_cell_size_hist=np.zeros(ncells)
    for cell in range(ncells):
        if RFP_cell[cell]>RThreshold:
            dead_mask[masks==cell]=1
        else:
