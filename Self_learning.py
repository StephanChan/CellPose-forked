...
def main():
    ...
    input_img_args.add_argument('--dir',
                        default=r'C:\Users\ScanImage\CellCounting\For_test', type=str, help='folder containing data to run or train on.')
    ...
    #################################################### evaluating images
        tic = time.time()
        if len(args.dir) > 0:
            image_names = my_io.my_get_BF_image_files(args.dir, 
        ...
        for image_name in tqdm(image_names, file=tqdm_out):
            ...
            out = model.eval(BF, channels=channels, diameter=diameter,
                            ...
                            model_loaded=True,BF=BF,BF_check=1,my_mask=None)
            masks, flows = out[:2]
            if len(out) > 3:
                diams = out[-1]
