# -*- coding: utf-8 -*-
"""
Created on Thu Nov 17 11:59:51 2022
...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov 15 15:50:42 2022
...
def main():
    ...
    hardware_args.add_argument('--use_gpu', action='store_true', default=1, help='use gpu if torch with cuda installed')
    ...
    input_img_args.add_argument('--dir',
                        default=r'C:\Users\ScanImage\CellCounting\lipofuscin\forTrain', type=str, help='folder containing data to run or train on.')
    ...
    input_img_args.add_argument('--look_one_level_down', action='store_true', default=0, help='run processing on all subdirectories of current folder')
    ...
    algorithm_args.add_argument('--diameter', required=False, default=5., type=float, 
    ...
    algorithm_args.add_argument('--flow_threshold', default=1.9, type=float, help='flow error threshold, 0 turns off this optional QC step. Default: %(default)s')
    ...
    training_args.add_argument('--train', action='store_true', default=0, help='train network using images in dir')
    training_args.add_argument('--train_size', action='store_true', default=0, help='train size network at end of training')
    ...
    training_args.add_argument('--diam_mean',
                        default=15., type=float, help='mean diameter to resize cells to during training -- if starting from pretrained models it cannot be changed from 30.0')
    ...
    args.pretrained_model='cyto2'
    args.train=0
    # args.pretrained_model=r'C:\Users\ScanImage\CellCounting\For_train\models\try1.307735'
    if args.check_mkl:
    ...
    #################################################### evaluating images
        tic = time.time()
        if len(args.dir) > 0:
            image_names = my_io.get_image_files(args.dir, '', None,
                                            look_one_level_down=args.look_one_level_down)
            # print(image_names)
            # utils.sum_GFP_RFP_for_pred(image_names)
        ...
        for image_name in tqdm(image_names, file=tqdm_out):
            ...
            # image_name=os.path.join(base_dir,base_name[:-2]+f'merge.tif')
            image = my_io.imread(image_name)
            my_mask=np.zeros(image.shape,dtype=np.uint8)
            my_mask=image>1.5
            # plt.figure()
            # plt.imshow(my_mask)
            # image_name_BF=os.path.join(base_dir,base_name[:-2]+f'BF.tif')
            # BF = my_io.imread(image_name_BF)[:,:,0]
            out = model.eval(image, channels=channels, diameter=diameter,
                            ...
                            model_loaded=True,BF=None,BF_check=0,my_mask=my_mask)
            masks, flows = out[:2]
            if len(out) > 3:
                diams = out[-1]
            ...
            # plt.imshow(masks)
            # import pickle
            # with open(r'C:\Users\ScanImage\CellCounting\New folder\1\1-mask.npy','wb') as file_handle:
            #     pickle.dump(masks,file_handle)
        logger.info('>>>> completed in %0.3f sec'%(time.time()-tic))
    else:
    ########################################################################## training
        test_dir = None if len(args.test_dir)==0 else args.test_dir
        output = my_io.load_train_test_data(args.dir, test_dir, imf, args.mask_filter, args.unet, args.look_one_level_down)
